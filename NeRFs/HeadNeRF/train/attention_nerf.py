...
from models.attention_nerf import AttentionNeRF
...
class Network(nn.Module):
    def __init__(self, H, W, focal, near, far, chunk, intrinsic, N_samlpes, N_importance):
        ...
        self.face_nerf_coarse = AttentionNeRF(D=args.netdepth, W=args.netwidth, output_ch=self.output_ch,
                                              skips=self.skips, dim_latent=32, input_ch_views=input_ch_views,
                                              use_viewdirs=args.use_viewdirs)
        self.face_nerf_fine = AttentionNeRF(D=args.netdepth, W=args.netwidth, dim_latent=32, output_ch=self.output_ch,
                                            skips=self.skips, input_ch_views=input_ch_views,
                                            use_viewdirs=args.use_viewdirs)
        self.SelfAttention = SelfAttention(input_ch=args.dim_expr + args.dim_aud, attn_output_ch=256)
    ...
    def forward(self, inputs):
        ...
        return self.render_dynamic_face(H=raw_img.shape[1], W=raw_img.shape[1], focal=self.focal,
                                        expr=expr, poses=pose, latent_code=latent_code, render_poses=render_poses,
    ...
    def run_network(self, inputs, expr_input, viewdirs, aud_input, nerf_model, latent_code, netchunk=1024 * 64):
        ...
        for i in range(0, embeded.shape[0], netchunk):
            embeded_xyz = embeded[i:i + netchunk]
            embeded_dir = embeded_dirs[i:i + netchunk]
            aud = einops.repeat(aud_input, 'c -> b c', b=embeded_xyz.shape[0])
            expr = expr_input * 1 / 3
            expr = einops.repeat(expr, 'c -> b c', b=embeded_xyz.shape[0])
            attn_input = torch.cat([aud, expr], dim=-1)
            attn_output = self.SelfAttention(attn_input)

            nerf_input = torch.cat((attn_output, embeded_xyz, embeded_dir), dim=-1)
            output = nerf_model(nerf_input, latent_code)
...
def train():
    ...
    logger.info(
        f'N_rand: {N_rand} , batch_size: {args.batch_size}, sample_rate {args.sample_rate}, netchunk: {args.netchunk}')
    ...
    val_loader = DataLoader(dataset=dataset_val, batch_size=args.batch_size, shuffle=True, num_workers=1, generator=torch.Generator(device='cuda'))
    # 写入config文件
    write_config(args)
