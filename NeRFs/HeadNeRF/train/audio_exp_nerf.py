...
from models.audio_net import AudioNet, AudioAttNet, DeepSpeechAudNet
import logging
...
class GetData(Dataset):
    ...
    def __init__(self, data_dir, aud_file, mode, args, skip=1):
        ...
        for frame in self.meta['frames'][::skip]:
            ...
            parse_img = os.path.join(self.data_dir, 'parsing', str(frame['img_id']) + '.png')
            self.all_parse_imgs.append(parse_img)
    ...
    def sample_rays(self, pose, face_rect, target, bc_img, landmark, parse_img):
        ...
        coords_mesh = torch.stack(torch.meshgrid(torch.linspace(0, self.H - 1, self.H), torch.linspace(0, self.W - 1, self.W)), -1)  # (H, W, 2)
        coords = torch.reshape(coords_mesh, [-1, 2])  # (H * W, 2)

        # 这里是计算人脸的位置
        mouth_Winds = (coords[:, 0] >= min_x) & (coords[:, 0] <= max_x) & (coords[:, 1] >= min_y) & (coords[:, 1] <= max_y)

        rect_Winds = (coords[:, 0] >= face_rect[0]) & (coords[:, 0] <= face_rect[0] + face_rect[2]) & \
                     (coords[:, 1] >= face_rect[1]) & (coords[:, 1] <= face_rect[1] + face_rect[3])

        torso_part = (parse_img[:, :, 0] == 255) & (parse_img[:, :, 1] == 0) & (parse_img[:, :, 2] == 0)

        if not os.path.exists(os.path.join(args.vis_path, 'torso.jpg')):
            parse_img[~torso_part] = 0
            imageio.imwrite(os.path.join(args.vis_path, 'torso.jpg'), parse_img)

        coords_mouth = coords[mouth_Winds]  # mouth
        coords_rect = coords[rect_Winds & ~mouth_Winds]  # face
        coords_norect = coords[~rect_Winds]  # background
        coords_torso = coords_mesh[torso_part]  # torso 部分
        coords_torso = torch.reshape(coords_torso, [-1, 2])

        """ 计算射线采样位置 """
        # 1/4的射线采样在嘴部
        mouth_num = args.mouth_rays
        torso_num = args.torso_rays
        sample_num = self.args.N_rand - mouth_num - torso_num
        ...
        select_coords_mouth = coords_mouth[select_inds_mouth].long()

        # 采样Torso
        select_inds_torso = np.random.choice(coords_torso.shape[0], size=[torso_num], replace=False)
        select_coords_torso = coords_torso[select_inds_torso].long()
        ...
        select_coords = torch.cat((select_coords_rect, select_coords_norect, select_coords_mouth, select_coords_torso), dim=0).long()
...
class Network(nn.Module):
    def __init__(self, H, W, focal, near, far, chunk, intrinsic, N_samlpes, N_importance):
        ...
        self.face_nerf_coarse = FaceNeRF(D=args.netdepth, W=args.netwidth,
                                         ...
                                         dim_latent=32, dim_expr=args.dim_expr, input_ch_views=input_ch_views,
                                         use_viewdirs=args.use_viewdirs)

        self.face_nerf_fine = FaceNeRF(D=args.netdepth, W=args.netwidth,
                                       input_ch=input_ch, dim_aud=args.dim_aud, dim_latent=32, dim_expr=args.dim_expr,
                                       output_ch=self.output_ch, skips=self.skips,
                                       input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)

        self.aud_net = AudioNet(args.dim_aud, args.win_size)
        self.aud_att_net = AudioAttNet()
        self.ds_aud_net = DeepSpeechAudNet()

    def forward(self, inputs):
        x, global_step, dataset_size = inputs
        batch_rays, target_s, bg_img, auds, raw_img, pose, expr, latent_code, index = x
        ...
        aud_feature, expr_feature = auds[index], None
        if args.dim_aud > 29 and global_step >= args.nosmo_iters:
        ...
        elif args.dim_aud > 29:
            aud_feature = self.aud_net(aud_feature)
        else:
            pass
            aud_feature = self.ds_aud_net(aud_feature)
        ...
        return self.render_dynamic_face(H=raw_img.shape[1], W=raw_img.shape[1], focal=self.focal,
                                        expr=expr_feature, poses=pose, latent_code=latent_code, render_poses=render_poses,
                                        chunk=args.chunk, near=self.near, far=self.far, rays=batch_rays, bc_rgb=bg_img,
                                        aud_para=aud_feature, ndc=False)

    def batchify_rays(self, rays, bc_rgb, aud_para, poses, latent_code, expr, chunk=1024 * 32):
        """
    ...
    def render_rays(self, rays, bc_rgb, aud_para, poses, latent_code, expr,
        ...
        raw = self.run_network(pts, expr, viewdirs, aud_para, self.face_nerf_coarse, latent_code,
        ...
        if args.N_importance > 0:
            ...
            raw = self.run_network(pts, expr, viewdirs, aud_para, self.face_nerf_fine, latent_code,
                                   netchunk=args.netchunk)
    ...
    def render_dynamic_face(self, H, W, focal, expr, poses, latent_code, render_poses=None,
                            chunk=1024 * 32, near=0., far=1., rays=None,
                            bc_rgb=None, aud_para=None, ndc=False, use_viewdirs=args.use_viewdirs):
        global viewdirs
        ...
        # Render and reshape
        all_ret = self.batchify_rays(rays, bc_rgb, aud_para, poses=poses, latent_code=latent_code, expr=expr,
                                     chunk=chunk)
        for k in all_ret:
...
def train():
    ...
    logger.info(
        f'N_rand: {N_rand} , dim_aud: {args.dim_aud}, dim_expr {args.dim_expr}, gt_dirs:{args.gt_dirs}')
    ...
    logger.info(f'dataset_val length: {dataset_val.data_size}')
    ...
    latent_codes = torch.ones(dataset_train.data_size, 32, dtype=torch.float32).cuda()
    ...
    # 定义优化器和学习率变化器
    training_params = list(network.parameters())
    training_params.append(latent_codes)
    latent_codes.requires_grad = True

    optimizer = torch.optim.Adam(params=training_params, lr=args.lrate,
                                 betas=(0.9, 0.999))
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        nerf_coarse_weight = ckpt['network_fn_state_dict']
        nerf_fine_weight = ckpt['network_fine_state_dict']

        del nerf_fine_weight['pts_linears.0.weight'], nerf_coarse_weight['pts_linears.0.weight']
        del nerf_fine_weight['pts_linears.5.weight'], nerf_coarse_weight['pts_linears.5.weight']
        del nerf_fine_weight['views_linears.0.weight'], nerf_coarse_weight['views_linears.0.weight']

        network.face_nerf_coarse.load_state_dict(nerf_coarse_weight, strict=False)
        network.face_nerf_fine.load_state_dict(nerf_fine_weight, strict=False)
        network.aud_net.load_state_dict(ckpt['network_audnet_state_dict'], strict=False)
        network.aud_att_net.load_state_dict(ckpt['network_audattnet_state_dict'], strict=False)
    else:
    ...
    for epoch in trange(start, N_iters):
        for iter, data in enumerate(tqdm(train_loader)):
            batch_rays, target_s, bg_img, auds, raw_img, pose, expr, index = data
            latent_code = latent_codes[index]
            data = batch_rays, target_s, bg_img, auds, raw_img, pose, expr, latent_code, index
            rgb, _, _, weights, extras = network([data, global_step, dataset_train.data_size])
            ...
            if 'rgb0' in extras:
                ...
                loss = loss + img_loss0
            latent_code_loss = torch.norm(latent_code) * args.lc_weight
            ...
            if global_step % args.i_print == 0:
                logger.info(
                    f"[TRAIN] epoch: {epoch} Iter: {iter} LatentLoss: {latent_code_loss.item()}  PSNR: {psnr.item()} LR: {new_lrate}")
                ...
                torch_writer.add_scalar('train/latent_code_loss', latent_code_loss.item(), global_step=global_step)

            if global_step % (100 * args.i_print) == 0:
                network.eval()
                for val_i, data in enumerate(tqdm(val_loader)):
                    batch_rays, target_s, bg_img, auds, raw_img, pose, expr, index = data
                    data = batch_rays, target_s, bg_img, auds, raw_img, pose, expr, latent_codes[0], index
                    with torch.no_grad():
                ...
                pred_with_label = pred_with_label[[2, 1, 0], :, :]
            ...
            if global_step % args.i_weights == 0:
                path = os.path.join(basedir, expname, 'head.tar')
                torch.save({
