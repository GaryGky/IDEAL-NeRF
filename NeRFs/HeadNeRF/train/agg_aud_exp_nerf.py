...
from models.face_nerf_agg import FaceNeRFAgg
...
class Network(nn.Module):
    def __init__(self, H, W, focal, near, far, chunk, intrinsic, N_samlpes, N_importance):
        ...
        self.face_nerf_coarse = FaceNeRFAgg(D=args.netdepth, W=args.netwidth,
                                            input_ch=input_ch, dim_aud=args.dim_aud, dim_latent=32, dim_expr=79,
                                            dim_agg=64,
                                            output_ch=self.output_ch, skips=self.skips,
                                            input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
        self.face_nerf_fine = FaceNeRFAgg(D=args.netdepth, W=args.netwidth,
                                          input_ch=input_ch, dim_aud=args.dim_aud, dim_latent=32, dim_expr=79,
                                          dim_agg=64,
                                          output_ch=self.output_ch, skips=self.skips,
                                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
