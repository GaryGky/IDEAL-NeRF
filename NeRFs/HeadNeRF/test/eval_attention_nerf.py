...
class GetData(Dataset):
    ...
    def __init__(self, data_dir, aud_file, mode, args, skip=1):
        ...
        self.aud_features = np.load(aud_file)
    ...
    def sample_rays(self, pose, face_rect, target, bc_img, landmark):
        ...
        """ 计算射线采样位置 """
        # 1/4的射线采样在嘴部
        sample_num = self.args.N_rand // 4 * 3
        mouth_num = self.args.N_rand // 4 * 1  # 1/4的射线采样在嘴部
...
class Network(nn.Module):
    def __init__(self, H, W, focal, near, far, chunk, intrinsic, N_samlpes, N_importance):
        ...
        self.face_nerf_fine = AttentionNeRF(D=args.netdepth, W=args.netwidth, dim_latent=32, output_ch=self.output_ch,
                                            ...
                                            use_viewdirs=args.use_viewdirs)
        self.SelfAttention = SelfAttention(input_ch=args.dim_expr + args.dim_aud + input_ch_views+input_ch, attn_output_ch=256)
...
def eval():
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        latent_codes = ckpt['latent_codes']
    # network = nn.DataParallel(network)
    global_step = 0
    network.eval()
    ...
    vid_out = cv2.VideoWriter(os.path.join(savePath, f'V_{args.datadir[8:]}_{args.aud_file[-9:-4]}_N_ExpPose0.avi'),
    ...
    for val_i, data in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            if val_i % 10 == 0:
                if not cv2.imwrite(os.path.join(savePath, f'{args.datadir[8:]}_{args.aud_file[-9:-4]}_ExpPose_{val_i}.jpg'), rgb):
