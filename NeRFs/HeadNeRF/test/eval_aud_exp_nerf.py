...
class GetData(Dataset):
    ...
    def __init__(self, data_dir, aud_file, mode, args, skip=1):
        ...
        self.mode = mode
        self.meta_identity = None
        with open(os.path.join(data_dir, 'transforms_exp_{}.json'.format(mode)), 'r') as fp:
            self.meta_identity = json.load(fp)
        self.meta_expr = None
        with open(args.evalExpr_path, 'r') as fp:
            self.meta_expr = json.load(fp)
        self.all_imgs = []
        self.all_poses = []
        self.auds = []
        self.all_face_rects = []
        self.all_landmarks = []  # .lms
        self.aud_features = np.load(aud_file)  # aud.npy
        self.background_img = torch.tensor(imageio.imread(os.path.join(data_dir, 'bc.jpg')) / 255.0)  # bc.jpg
        self.all_exprs = []
        self.focal, self.cx, self.cy = float(self.meta_expr['focal_len']), float(self.meta_identity['cx']), float(
            self.meta_identity['cy'])
        self.H, self.W = int(self.cy * 2), int(self.cx * 2)
        self.args = args
        # 用于valSet
        self.skip = args.testskip

        for frame in self.meta_identity['frames'][::skip]:
        ...
        # 从头开始取音频
        for aud_id in range(len(self.all_imgs)):
            self.auds.append(self.aud_features[aud_id])

        # Pose和Expression 从测试的对象中获取
        for frame in self.meta_expr['frames'][::skip]:
            self.all_exprs.append(frame['exp'])
            if len(self.all_exprs) >= len(self.all_imgs):
                break
...
def eval():
    # Def: 超参
    H, W, focal, cx, cy, = 450, 450, 1200, 225.0, 225.0
    intrinsic = np.array([[focal, 0., W / 2], [0, focal, H / 2], [0, 0, 1.]])
    basedir, expname = args.basedir, args.expname

    logger.info(
        f'basedir: {args.basedir}, expname: {args.expname}, datadir: {args.datadir}, audfile: {args.aud_file}, testskip: {args.testskip}')

    logger.info(f'near: {args.near}, far: {args.far}, use_viewdirs={args.use_viewdirs}')
    ...
    latent_codes = torch.zeros(1, 32, dtype=torch.float32)
    # 加载保存的模型
