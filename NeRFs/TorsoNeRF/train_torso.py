...
from NeRFs.TorsoNeRF.run_nerf_helpers import *
from NeRFs.TorsoNeRF.run_nerf import raw2outputs
...
embed_torso_aud_fn, input_ch_torso_aud = get_embedder(3, 0)  # embed_out_dim = 21
...
class GetData(Dataset):
    ...
    def __init__(self, data_dir, aud_file, mode, args, skip=1):
        ...
        self.auds = []
        self.all_face_rects = []
        self.all_landmarks = []  # .lms
        self.aud_features = np.load(os.path.join(self.data_dir, aud_file))
        self.background_img = torch.tensor(
            imageio.imread(os.path.join(data_dir, 'bc.jpg')) / 255.0).cuda()
        self.all_exprs = []
        ...
        for frame in self.meta['frames'][::skip]:
            fname = os.path.join(self.data_dir, args.gt_dirs, str(frame['img_id']) + '.jpg')
            ...
            self.all_exprs.append(frame['exp'])

        # 数据集的长度
        self.data_size = len(self.all_imgs) if mode == 'train' else len(self.all_imgs) // args.testskip

        self.auds = torch.tensor(self.auds, dtype=torch.float)
    ...
    def __getitem__(self, index):
        ...
        batch_rays, batch_rays_torso, target_s, bc_rgb = self.sample_rays(pose, rect, target, self.background_img,
                                                                          landmark)
        # bc_rgb是通过射线采样得到的
        bc_rgb = bc_rgb if self.mode == 'train' else self.background_img
        exp = torch.tensor(self.all_exprs[index], dtype=torch.float32)

        return batch_rays, batch_rays_torso, target_s, bc_rgb, self.auds, raw_img, pose, exp, index
    ...
    def sample_rays(self, pose, face_rect, target, bc_img, landmark):
        rays_o, rays_d = self.get_rays(self.H, self.W, self.focal, torch.Tensor(pose).cuda(), self.cx, self.cy)
        pose_torso = self.all_poses[0][:3, :4]
        torso_rays_o, torso_rays_d = self.get_rays(self.H, self.W, self.focal, torch.Tensor(pose_torso).cuda(), self.cx,
                                                   self.cy)
        # 这里计算嘴部的位置
        ...
        """ 计算射线采样位置 """
        # 1/4的射线采样在嘴部
        mouth_num = self.args.N_rand // 4 * 1 if args.use_highlight else 0
        sample_num = self.args.N_rand - mouth_num
        ...
        torso_rays_d = torso_rays_d[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)
        torso_rays_o = torso_rays_o[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)
        batch_rays_torso = torch.stack([torso_rays_o, torso_rays_d], 0)

        target_s = target[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)
        bc_rgb = bc_img[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)

        return batch_rays, batch_rays_torso, target_s, bc_rgb
...
class Network(nn.Module):
    def __init__(self, H, W, focal, near, far, chunk, N_samlpes, N_importance):
        ...
        self.face_nerf_coarse = FaceNeRF(D=args.netdepth, W=args.netwidth,
                                         ...
                                         dim_latent=32, dim_expr=79, input_ch_views=input_ch_views,
                                         use_viewdirs=args.use_viewdirs)
        self.face_nerf_fine = FaceNeRF(D=args.netdepth, W=args.netwidth,
                                       input_ch=input_ch, dim_aud=args.dim_aud, dim_latent=32, dim_expr=79,
                                       output_ch=self.output_ch, skips=self.skips,
        ...
        self.torso_coarse_nerf = FaceNeRF(D=args.netdepth, W=args.netwidth,
                                          input_ch=input_ch, dim_aud=args.dim_aud_body + 2 * input_ch_torso_aud,
                                          output_ch=self.output_ch, skips=self.skips,
                                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)

        self.torso_fine_nerf = FaceNeRF(D=args.netdepth, W=args.netwidth,
                                        input_ch=input_ch, dim_aud=args.dim_aud_body + 2 * input_ch_torso_aud,
                                        output_ch=self.output_ch, skips=self.skips,
                                        input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
    ...
    def forward(self, inputs):
        ...
        batch_rays, batch_rays_torso, target_s, bg_img, auds, raw_img, pose, expr, latent_code, index = x

        batch_rays, bg_img, auds, pose, expr, latent_code = \
            torch.squeeze(batch_rays), torch.squeeze(bg_img), torch.squeeze(
                auds), torch.squeeze(pose), torch.squeeze(expr), torch.squeeze(latent_code)
        batch_rays, target_s, bg_img, auds, raw_img, pose, expr = batch_rays.type(
            ...
            torch.FloatTensor).cuda(), pose.type(torch.FloatTensor).cuda(), expr.type(torch.FloatTensor).cuda()
        batch_rays_torso = torch.squeeze(batch_rays_torso).type(torch.FloatTensor).cuda()

        aud_feature = self.aud_net(auds[index])
        et = pose_to_euler_trans(pose.unsqueeze(0))
        embed_et = torch.cat((embed_torso_aud_fn(et[:, :3]), embed_torso_aud_fn(et[:, 3:])), dim=1)
        aud_torso_feature = torch.cat((aud_feature[..., :args.dim_aud_body], torch.squeeze(embed_et)), dim=-1)

        render_poses = None if self.training is True else pose[:3, :4]

        network_nerf = {'fine': self.face_nerf_fine, 'coarse': self.face_nerf_coarse}

        # 渲染头部
        rgb, _, _, last_weight, rgb_fg, extras = self.render_dynamic_face(H=self.H, W=self.W,
                                                                          focal=self.focal,
                                                                          expr=expr, latent_code=latent_code,
                                                                          render_poses=render_poses,
                                                                          chunk=args.chunk, near=self.near, far=self.far,
                                                                          rays=batch_rays, bc_rgb=bg_img,
                                                                          aud_para=aud_feature, network_nerf=network_nerf,
                                                                          ndc=False)
        # 渲染躯干
        network_nerf = {'fine': self.torso_fine_nerf, 'coarse': self.torso_coarse_nerf}
        rgb_torso, _, _, last_weight_torso, rgb_fg_torso, extras_torso = self.render_dynamic_face(H=self.H,
                                                                                                  W=self.W,
                                                                                                  focal=self.focal,
                                                                                                  expr=None,
                                                                                                  latent_code=None,
                                                                                                  ...
                                                                                                  rays=batch_rays_torso, bc_rgb=bg_img,
                                                                                                  aud_para=aud_torso_feature,
                                                                                                  network_nerf=network_nerf,
                                                                                                  ndc=False)
        rgb_com = rgb * last_weight_torso[..., None] + rgb_fg_torso
        rgb_com0 = extras['rgb0'] * extras_torso['last_weight0'][..., None] + extras_torso['rgb_map_fg0']
        return rgb_com, rgb_com0

    def batchify_rays(self, rays, bc_rgb, aud_para, latent_code, expr, chunk=1024 * 32, network_nerf=None):
        """
            Render rays in smaller minibatches to avoid OOM. -> 异步地加入GPU不能防止OOM，实现不了想要的效果
            这里用主要是为了防止evaluation的时候一次性往显存中加入太多的信息
        """
        all_ret = {}
        for i in range(0, rays.shape[0], chunk):
            ret = self.render_rays(rays[i:i + chunk], bc_rgb[i:i + chunk], aud_para, latent_code, expr,
                                   network_nerf=network_nerf)
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
    ...
    def render_rays(self, rays, bc_rgb, aud_para, latent_code, expr,
                    retraw=False, lindisp=False, perturb=args.perturb, white_bkgd=False, network_nerf=None,
        ...
        raw = self.run_network(pts, expr, viewdirs, aud_para, network_nerf['coarse'], latent_code, netchunk=args.netchunk)

        rgb_map0, disp_map0, acc_map0, weights0, depth_map0, rgb_map_fg0 = self.raw2outputs(
            raw, z_vals, rays_d, bc_rgb, raw_noise_std, white_bkgd, pytest=pytest)

        # if args.N_importance > 0:
        last_weight_0 = weights0[..., -1]
        z_vals_mid = .5 * (z_vals[..., 1:] + z_vals[..., :-1])
        z_samples = sample_pdf(
            z_vals_mid, weights0[..., 1:-1], args.N_importance, det=(perturb == 0.), pytest=pytest)
        ...
        raw = self.run_network(pts, expr, viewdirs, aud_para, network_nerf['fine'], latent_code, netchunk=args.netchunk)

        rgb_map, disp_map, acc_map, weights, depth_map, rgb_map_fg = self.raw2outputs(
            raw, z_vals, rays_d, bc_rgb, raw_noise_std, white_bkgd, pytest=pytest)
    ...
    def run_network(self, inputs, expr, viewdirs, aud, nerf_model, latent_code, netchunk=1024 * 64):
        ...
        for i in range(0, embeded.shape[0], netchunk):
            output = nerf_model(embeded[i:i + netchunk], aud, expr, latent_code)
    ...
    def render_dynamic_face(self, H, W, focal, expr, latent_code, render_poses=None,
                            chunk=1024 * 32, near=0., far=1., rays=None,
                            bc_rgb=None, aud_para=None, network_nerf=None, ndc=False, use_viewdirs=args.use_viewdirs):
        global viewdirs
        ...
        # Render and reshape
        all_ret = self.batchify_rays(rays, bc_rgb, aud_para, latent_code=latent_code, expr=expr, chunk=chunk,
                                     network_nerf=network_nerf)
        for k in all_ret:
...
def train():
    ...
    logger.info(
        f'N_rand: {N_rand} , batch_size: {args.batch_size}, sample_rate {args.sample_rate}')
    ...
    focal = dataset_val.focal
    # 写入config文件
    write_config(args)

    # 定义网络模型
    network = Network(H, W, focal, near=args.near, far=args.far, chunk=args.chunk,
                      N_samlpes=args.N_samples, N_importance=args.N_importance)
    latent_codes = torch.ones(dataset_train.data_size, 32, dtype=torch.float32, requires_grad=True).cuda()

    network.apply(init_weights)

    # 定义优化器和学习率变化器
    training_params = list(network.torso_coarse_nerf.parameters()) + list(network.torso_fine_nerf.parameters()) + \
                      list(network.aud_net.parameters()) + list(network.aud_att_net.parameters())

    optimizer = torch.optim.Adam(params=training_params, lr=args.lrate, betas=(0.9, 0.999))
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        if len(ckpts) > 0:
            ckpt_path = ckpts[-1]
            logger.info(f'Found ckpts:{ckpt_path}')
            ckpt = torch.load(ckpt_path)
            network.load_state_dict(ckpt['model_state_dict'], strict=False)
            latent_codes.data = ckpt['latent_codes']

        ckpts = [os.path.join(basedir, expname, f) for f in natsorted(
            os.listdir(os.path.join(basedir, expname))) if 'torso.tar' in f]
        if len(ckpts) > 0:
            ckpt_path = ckpts[-1]
            logger.info(f'Found ckpts:{ckpt_path}')
            ckpt = torch.load(ckpt_path)
    ...
    network.face_nerf_fine.eval()
    network.face_nerf_coarse.eval()
    for epoch in trange(start, N_iters):
        for iter, data in enumerate(tqdm(train_loader)):
            batch_rays, batch_rays_torso, target_s, bg_img, auds, raw_img, pose, expr, index = data
            latent_code = latent_codes[index]
            data = batch_rays, batch_rays_torso, target_s, bg_img, auds, raw_img, pose, expr, latent_code, index

            rgb, rgb0 = network([data, global_step, dataset_train.data_size])
            ...
            img_loss0 = F.mse_loss(rgb0, target_s)
            loss = loss + img_loss0
            ...
            if global_step % args.i_print == 0:
                logger.info(
                    f"[TRAIN] epoch: {epoch} Iter: {iter}  PSNR: {psnr.item()} LR: {new_lrate}")
                torch_writer.add_scalar('train/loss', loss.item(), global_step=global_step)
                torch_writer.add_scalar('train/psnr', psnr.item(), global_step=global_step)
                torch_writer.add_scalar('train/learning_rate', new_lrate, global_step=global_step)

            if global_step % (100 * args.i_print) == 0:
                network.eval()
                for val_i, data in enumerate(tqdm(val_loader)):
                    batch_rays, batch_rays_torso, target_s, bg_img, auds, raw_img, pose, expr, index = data
                    data = batch_rays, batch_rays_torso, target_s, bg_img, auds, raw_img, pose, expr, latent_codes[0], index
                    with torch.no_grad():
                        rgb, rgb0 = network([data, global_step, dataset_val.data_size])
                        break
            ...
            if global_step % args.i_weights == 0:
                path = os.path.join(basedir, expname, '{:06d}_torso.tar'.format(epoch))
                torch.save({
