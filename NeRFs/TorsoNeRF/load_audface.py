...
def load_test_data(datadir, aud_file, test_pose_file='dataset/Obama/transforms_val.json',
                   testskip=1, test_size=-1, aud_start=0):
    with open(os.path.join(test_pose_file)) as fp:
        meta = json.load(fp)
    poses = []
    auds = []
    aud_features = np.load(os.path.join(aud_file))
    aud_ids = []
    cur_id = 0
    for frame in meta['frames'][::testskip]:
        poses.append(np.array(frame['transform_matrix']))
