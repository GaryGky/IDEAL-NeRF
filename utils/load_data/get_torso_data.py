...
class GetData(Dataset):
    ...
    def __init__(self, data_dir, aud_file, mode, args, skip=1):
        ...
        for frame in self.meta['frames'][::skip]:
            fname = os.path.join(self.data_dir, 'com_imgs', str(frame['img_id']) + '.jpg')
            landmark = os.path.join(self.data_dir, 'ori_imgs', str(frame['img_id']) + '.lf')
    ...
    def __getitem__(self, index):
        ...
        rect = self.all_face_rects[index]
        ...
        return batch_rays, target_s, bc_rgb, torch.tensor(self.auds, dtype=torch.float), raw_img, pose, landmark, index
